...
(int rounds, int score) ParseBoard(string data, List<int> drawings)
{
    ...
    var rowRounds = rows.Select(row => row.Select(v => drawings.IndexOf(v))
                            .Select(i => i < 0?1000:i).Max()).Min();
    var cols = Enumerable.Range(0, rows.Count())
                    .Select(i => rows.Select(r => r.ElementAt(i)).ToArray()).ToList();
    var colRounds = cols.Select(col => col.Select(v => drawings.IndexOf(v))
                            .Select(i => i < 0?1000:i).Max()).Min();
