...
remaining (Block (x1, x2) y z) o@(Block (a1, a2) b c) | x1 < a1 =
    Block (x1, a1-1) y z:remaining (Block (a1, x2) y z) o
remaining (Block (x1, x2) y z) o@(Block (a1, a2) b c) | x2 > a2 =
    Block (a2+1, x2) y z:remaining (Block (x1, a2) y z) o
remaining (Block x (y1, y2) z) o@(Block a (b1, b2) c) | y1 < b1 =
    Block x (y1, b1-1) z:remaining (Block x (b1, y2) z) o
remaining (Block x (y1, y2) z) o@(Block a (b1, b2) c) | y2 > b2 =
    Block x (b2+1, y2) z:remaining (Block x (y1, b2) z) o
remaining (Block x y (z1, z2)) o@(Block a b (c1, c2)) | z1 < c1 =
    Block x y (z1, c1-1):remaining (Block x y (c1, z2)) o
remaining (Block x y (z1, z2)) o@(Block a b (c1, c2)) | z2 > c2 =
    Block x y (c2+1, z2):remaining (Block x y (z1, c2)) o
remaining _ _ = []
